...
const usePaymentTypeStore = defineStore('PaymentTypeStore', {
  state: () => ({
    form: {
      id: null,
      name: '',
  ...
  actions: {
    async show() {
      try {
        const response = await useMyFetch().post(
          '/v1/payment-types/show-payment-type',
          this.form
        )
    ...
    async store() {
      ...
      try {
        const response = await useMyFetch().post(
          '/v1/payment-types/create-payment-type',
          this.form
        )
        ...
        navigateTo(`/masters/customizations/payment-types/edit/${response.data.data[0].id}`)
    ...
    async update() {
      ...
      try {
        ...
        const response = await useMyFetch().post(
          '/v1/payment-types/update-payment-type',
          this.form
        )
        this.form = JSON.parse(
          JSON.stringify(useInitials.formPurchaseTypeCreateEdit)
    ...
    async delete(id: number | string | string[] | undefined) {
      ...
      try {
        const response = await useMyFetch().post(
          '/v1/payment-types/delete-payment-type',
          this.form
        )
        this.form = response.data.data[0]
    ...
    async restore(id: number | string | string[] | undefined) {
      ...
      try {
        const response = await useMyFetch().post(
          '/v1/payment-types/restore-payment-type',
          this.form
        )
        this.form = response.data.data[0]
