...
import type { FormPurchaseType } from '~/types/masters/PurchaseType'

const usePurchaseTypeStore = defineStore('PurchaseTypeStore', {
  state: () => ({
    form: {
      id: null,
      name: '',
  ...
  actions: {
    async show() {
      try {
        const response = await useMyFetch().post(
          '/v1/purchase-types/show-purchase-type',
          this.form
        )
    ...
    async store() {
      ...
      try {
        const response = await useMyFetch().post(
          '/v1/purchase-types/create-purchase-type',
          this.form
        )
        this.form = JSON.parse(
          JSON.stringify(useInitials.formPurchaseTypeCreateEdit)
        )

        useAlert.hideAlert()
        useAlert.alertSuccess(response.data.message)
        navigateTo(`/masters/customizations/purchase-types/edit/${response.data.data[0].id}`)
    ...
    async update() {
      ...
      try {
        ...
        const response = await useMyFetch().post(
          '/v1/purchase-types/update-purchase-type',
          this.form
        )
        this.form = JSON.parse(
          JSON.stringify(useInitials.formPurchaseTypeCreateEdit)
    ...
    async delete(id: number | string | string[] | undefined) {
      ...
      try {
        const response = await useMyFetch().post(
          '/v1/purchase-types/delete-purchase-type',
          this.form
        )
        this.form = response.data.data[0]
    ...
    async restore(id: number | string | string[] | undefined) {
      ...
      try {
        const response = await useMyFetch().post(
          '/v1/purchase-types/restore-purchase-type',
          this.form
        )
        this.form = response.data.data[0]
...
export default usePurchaseTypeStore
