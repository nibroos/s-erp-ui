  ...
  <div class="flex flex-col gap-2 p-4 bg-white dark:bg-dark2">
    ...
    <div class="grid grid-cols-4 items-center gap-2">
      ...
      <d-submit-button
        ...
        @click:clear="dashboardStore.clearLineFilter()"
        class="grid-cols-1"
      />
    </div>

    <div class="flex flex-col items-center justify-between gap-2 w-full">
      <AgCharts :options="barOptions" class="w-full" />
      <div
        class="flex gap-2 grow justify-between w-full"
        v-if="!metaModal.indexDonut.loading"
      >
        <div
          v-for="(item, index) in metaModal.indexDonut.data"
          :key="index"
          class="grow"
        >
          <div
...
const onClickFilter = async () => {
  ...
  // await dashboardStore.indexSalesOrderLine().then(() => {
  //   // barOptions.value.data = metaModal.value.indexDonut.data;
  //   // take other than "TOTAL"
  //   filteredData = metaModal.value.indexDonut.data;
  //   barOptions.value = {
  //     ...barOptions.value,
  //     data: filteredData,
  //   };
