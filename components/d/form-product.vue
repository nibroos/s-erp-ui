...
const handleSubmit = async () => {
  ...
  if (!!props.id) {
    ...
    await productStore.updateModal().then((res) => {
      isOpen.value = false;
      emits("submit:form", res);
    });
  } else {
    await productStore.storeModal().then((res) => {
      isOpen.value = false;
      emits("submit:form", res);
    });
  }
...
const labelForm = ref("Add Product/Item");
...
onMounted(async () => {
  ...
  if (!!props.id) {
    form.value.id = Number(props.id);
    labelForm.value = "Edit Product/Item";
                ...
                <template #item.conversion="{ item }">
                  <div class="flex w-full justify-end">
                    <d-num-v-format
                      ...
                      label=""
                      class="w-[9rem]"
                      :disabled="form.item_unit_id === item.unit_id"
                    />
                  </div>
                </template>
                <template #item.price_buy="{ item }">
