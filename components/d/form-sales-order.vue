...
const handleSubmit = async () => {
  ...
  if (!!props.id) {
    ...
    await salesOrderStore.updateModal().then((res) => {
      isOpen.value = false;
      emits("submit:form", res);
    });
  } else {
    await salesOrderStore.storeModal().then((res) => {
      isOpen.value = false;
      emits("submit:form", res);
    });
  }
...
const openCloseModalForm = (state: boolean) => {
  isOpen.value = state;
};
...
onMounted(async () => {
  ...
  if (!!props.id) {
    form.value.id = Number(props.id);
    labelForm.value = "Edit Sales Order";
  ...
  <div>
    ...
    >
      <div class="flex flex-col gap-2">
          ...
          <template #title-append>
            <d-select-table
              ...
              @click:selected="
                (data) => {
                  if (!!data) {
                    salesOrderStore.goToSalesOrder(data.id);
                  }
                }
              "
                    ...
                    <td :colspan="columns.length" class="!p-0">
                      <div class="">
                        ...
                        >
                          ...
                          <template
                            #item.action="{ item: itemBom, index: iBom }"
                          >
                  ...
                  <div class="lg:col-span-6 col-span-2 flex gap-2 items-center">
                    ...
                    >
                      <template v-slot:activator="{ props }">
                        ...
                        >
                          <span
                            :class="classMerge('text-xs dark:text-primary1')"
                            >Color</span
                          >
                          <div
                            :style="{
                      ...
                      >
                        ...
                        <div class="flex gap-2">
                          <d-bt
                            ...
                            @click="
                              salesOrderStore.handleViewFullPageFile(file)
                            "
                          ></d-bt>
                          <d-bt
                            icon="mdi-download"
                          ...
                          <d-bt
                            @click="
                              salesOrderStore.handleExistingFile(file, index)
                            "
                            icon="mdi-delete"
                            is-no-text
                            class="p-1 bg-primary1 hover:text-zinc-100 hover:bg-lightCancel2 rounded-full ease-in-out transition-all hover:dark:!bg-cancel1 dark:!bg-cancel"
        ...
        >
          <template #header-end>
            <d-form-product @submit:form="salesOrderStore.fetchModalFilter()" />
          </template>
          <template #top>
            <form
          ...
          <template #footer>
            <div class="flex h-max w-full justify-end items-center gap-2">
              <d-form-product
                @submit:form="salesOrderStore.fetchModalFilter()"
              />
              <button
        ...
        >
          ...
          >
            ...
            >
              ...
              <tr
                v-else-if="!!item.quo_dts_boms && item.quo_dts_boms.length > 0"
              >
                <td :colspan="columns.length" class="!p-0">
                  <div class="">
                    <v-data-table-virtual
