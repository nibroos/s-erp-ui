...
const handleSubmit = async () => {
  ...
  if (!!props.id) {
    ...
    await quotationStore.updateModal().then((res) => {
      isOpen.value = false;
      emits("submit:form", res);
    });
  } else {
    await quotationStore.storeModal().then((res) => {
      isOpen.value = false;
      emits("submit:form", res);
    });
  }
...
const labelForm = ref("Add Quotation");

onMounted(async () => {
  clickClearForm();
  if (!!props.id) {
    form.value.id = Number(props.id);
    labelForm.value = "Edit Quotation";
      ...
      <div class="flex flex-col gap-2">
        ...
        >
          ...
          <template #title-append>
            <div
              class="rounded-lg p-1 bg-grey2 dark:text-primary1 dark:bg-grey3 text-sm"
            >
              #{{ form.quo_no }}
            </div>
