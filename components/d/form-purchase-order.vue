...
const handleSubmit = async () => {
  ...
  if (!!props.id) {
    ...
    await purchaseOrderStore.updateModal().then((res) => {
      isOpen.value = false;
      emits("submit:form", res);
    });
  } else {
    await purchaseOrderStore.storeModal().then((res) => {
      isOpen.value = false;
      emits("submit:form", res);
    });
  }
...
const labelForm = ref("Add Purchase Order");
...
onMounted(async () => {
  ...
  if (!!props.id) {
    form.value.id = Number(props.id);
    labelForm.value = "Edit Purchase Order";
      ...
      <div class="flex flex-col gap-2">
        ...
        >
          ...
          <template #content>
            ...
            <div
              v-if="tabFormIndex == useStatics.formTabPurchaseOrder.attachments"
            >
        ...
        >
          <template #header-end>
            <d-form-product
              @submit:form="purchaseOrderStore.fetchModalFilter()"
            />
          </template>
          <template #top>
            <form
