...
const handleSubmit = async () => {
  ...
  if (!!props.id) {
    ...
    await inventoryStore.updateModal().then((res) => {
      isOpen.value = false;
      emits("submit:form", res);
    });
  } else {
    await inventoryStore.storeModal().then((res) => {
      isOpen.value = false;
      emits("submit:form", res);
    });
  }
...
onMounted(async () => {
  ...
  if (!!props.id) {
    form.value.id = Number(props.id);
    labelForm.value = "Edit Inventory IN";
        ...
        >
          <template #header-end>
            <d-form-purchase-order
              @submit:form="inventoryStore.fetchModalFilter()"
            />
          </template>
          <template #top>
            <form
