...
const handleSubmit = async () => {
  await companyProfileStore.store();
};

const handleClickClear = () => {
  form.value = cloneObject(useInitials.formCompanyProfileCreateEdit);
  errors.value = {};
  logoPreview.value = '';
      ...
      <template #content>
        <div>
          <div class="flex justify-between items-center mb-4">
            <h2 class="text-lg font-semibold text-[#747c83]">Bank Information</h2>
            <button 
              type="button" 
              class="px-4 py-1.5 bg-[#695149] text-white rounded text-[14.5px] hover:bg-[#4d3a34]"
              @click="addBankInfo"
            >
              + Add Bank
            </button>
