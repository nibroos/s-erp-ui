...
import useShippingTermStore from "~/stores/masters/ShippingTermStore";
import type { FormLayoutType } from "~/types/FormLayoutType";

const shippingTermStore = useShippingTermStore();
const { tabFormIndex, form, errors } = storeToRefs(shippingTermStore);
...
const formLayout = ref({
  ...
  parentPath: "/masters/customizations/shipping-terms",
  mode: "edit",
  ...
  button: {
    create: {
      show: true,
      cta: "Create New",
      path: "/masters/customizations/shipping-terms/create",
    },
    save: {
      show: true,
      loading: false,
...
const handleSubmit = async () => {
  await shippingTermStore.update();
};

const handleClickClear = () => {
  form.value = cloneObject(useInitials.formShippingTermCreateEdit);
  errors.value = {};
};
...
onMounted(async () => {
  ...
  Promise.all([shippingTermStore.show()]);
});
</script>
