...
import usePaymentTermStore from "~/stores/masters/PaymentTermStore";
import type { FormLayoutType } from "~/types/FormLayoutType";

const paymentTermStore = usePaymentTermStore();
const { tabFormIndex, form, errors } = storeToRefs(paymentTermStore);
...
const formLayout = ref({
  ...
  parentPath: "/masters/customizations/payment-terms",
  mode: "edit",
  ...
  button: {
    create: {
      show: true,
      cta: "Create New",
      path: "/masters/customizations/payment-terms/create",
    },
    save: {
      show: true,
      loading: false,
...
const handleClickClear = () => {
  form.value = cloneObject(useInitials.formPaymentTermCreateEdit);
  errors.value = {};
};
...
onMounted(async () => {
  ...
  Promise.all([paymentTermStore.show()]);
});
</script>
