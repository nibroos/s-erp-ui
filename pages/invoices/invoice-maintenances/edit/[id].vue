...
useHead({
  title: "Edit Invoice Maintenance",
});
...
const nextInvoiceMaintenance = ref<number | null>(null);
...
const formLayout = ref({
  ...
  button: {
    create: {
      path: "/invoices/invoice-maintenances/create",
      show: true,
    },
    save: {
      show: true,
...
const handleSubmit = async () => {
  await invoiceMaintenanceStore.update();
};
...
onMounted(async () => {
  form.value.id = id;
  await invoiceMaintenanceStore.show();
  initialFormLayout();
  await invoiceMaintenanceStore.fetchVatOptions();
  await fetchNextInvoiceMaintenance();
...
<template>
  <div class="flex flex-col gap-2">
    <d-form-layout
      ...
      @click:pdf="invoiceMaintenanceStore.onClickPDF()"
    >
      <!-- <template #title-append>
        <d-select-table
          ...
          cta="Go To Inv Maintenance"
          v-model="nextInvoiceMaintenance"
          class="col-span-2 lg:col-span-1"
          is-quick-select
          @click:selected="
            (data) => {
              if (!!data) {
                invoiceMaintenanceStore.goToInvoiceMaintenance(data.id);
              }
            }
          "
              ...
              >
                <button
                  v-if="(internalItem.raw.product_type === 'product' || (internalItem.raw as any).item_type === 'product') && 
                        ...
                        ((internalItem.raw as any).so_dts_boms && (internalItem.raw as any).so_dts_boms.length > 0))"
    ...
    >
      ...
      >
        ...
        >
          <tr v-if="rowItem.so_dts_boms && rowItem.so_dts_boms.length > 0">
            <td :colspan="columns.length" class="!p-0">
              <div class="">
                <v-data-table-virtual
                  :headers="headersBom"
                  :items="rowItem.so_dts_boms || []"
                  item-value="uid"
                  density="compact"
                  return-object
                  fixed-header
                  class="table-hover"
                  :height="rowItem.so_dts_boms.length > 1 ? '170' : '100'"
                  :header-props="{
                    class: '!bg-grey1 dark:!bg-dark2 whitespace-nowrap',
                  }"
                  :row-props="{
