...
const { queryModal, metaModal, tabIndex } = storeToRefs(invoiceDpStore);
...
const fetchFilter = async () => {
  await useInvoiceDpStore().indexInvoiceDpDetails();
};
...
const onClickFind = async (filters: QInvoiceDpIndexType) => {
  ...
  await useInvoiceDpStore().indexWidget();
};
  ...
  <div class="flex flex-col gap-2">
    ...
    >
      <d-datatable
        ...
        csv-api="/v1/invoice-dps/csv-invoice-dp"
        :fields="fieldsConfig"
        ...
        <template #tab.content.detail>
          ...
          >
            ...
            <template #item.item_name="{ item }: { item: any }">
              <div
                v-for="(product, iProduct) in item.invoice_dp_dts"
                :key="iProduct"
              ...
              >
                ...
                <div
                  v-for="(soDtBom, iSoDtBom) in product.invoice_dp_dt_boms"
                  :key="iSoDtBom"
                  class="whitespace-nowrap align-top"
                >
                  <br v-if="iSoDtBom < product.invoice_dp_dt_boms.length - 1" />
                </div>
              </div>
            </template>
            ...
            <template #item.bom_item_name="{ item }: { item: any }">
              <div
                ...
                class="whitespace-nowrap align-top"
              >
                <div v-if="product.invoice_dp_dt_boms.length === 0">-</div>
                ...
                <d-divider v-if="iProduct != item.invoice_dp_dts.length - 1" />
              </div>
            </template>
