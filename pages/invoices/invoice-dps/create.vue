...
const formLayout = ref({
  ...
  tabs: useStatics.formTabInvoiceDp
    ? Object.keys(useStatics.formTabInvoiceDp).map(
...
const calculateTotalAmountLocal = () => {
  ...
  if (invoiceDpStore.currencySymbolLabel) {
    const symbol = invoiceDpStore.currencySymbolLabel;
    customSummary.value.sub_amount.symbol = symbol;
    customSummary.value.total_amount.symbol = symbol;
...
onMounted(async () => {
  invoiceDpStore.handleClickClear();
  initialFormLayout();
        ...
        >
            ...
            <d-autocomplete
              ...
              @click:selected="
                (data: FormCurrencyType) => {
                  invoiceDpStore.autocompleteCurrency(data);
                  calculateTotalAmountLocal();
                }
              "
            ></d-autocomplete>
          ...
          <div class="lg:col-span-6">
            <d-autocomplete
              ...
              @click:selected="
                (data: FormPph23Type) => {
                  invoiceDpStore.autocompletePph(data);
                  calculateTotalAmountLocal();
                }
              "
              @click:clear="
                () => {
                  invoiceDpStore.removePph();
                  calculateTotalAmountLocal();
                }
              "
            ></d-autocomplete>
          ...
          <div class="lg:col-span-6 flex gap-2">
            <d-switch-status
              ...
              @update:model-value="
                async (value) => {
                  await invoiceDpStore.onClickSwitchVAT(value);
                  calculateTotalAmountLocal();
                }
              "
            />
        ...
        <div v-if="tabFormIndex == useStatics.formTabInvoiceDp.items">
          <div class="grid grid-cols-3 sm:grid-cols-1 gap-2">
            <d-option-ref-btn
              ...
              class="col-span-2"
              @click:ref="
                (ref) => invoiceDpStore.onClickOpenModalOptionRefBtn(ref)
              "
            ></d-option-ref-btn>
