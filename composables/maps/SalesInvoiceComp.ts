...
import type { FormSalesInvoiceDtProductListType, FormSalesInvoiceDtRefType, SalesInvoiceDtBomType, SalesInvoiceDtType, SalesInvoiceRefType } from "~/types/sales-invoices/SalesInvoiceType"

export const convertSalesInvoiceItemRefProduct = (
  item: FormSalesInvoiceDtProductListType,
  refType: SalesInvoiceRefType
): SalesInvoiceDtType => {
  ...
  let bomItems: SalesInvoiceDtBomType[] = [];
  if (item.so_dts_boms && item.so_dts_boms.length > 0) {
    bomItems = item.so_dts_boms.map((bomItem: any): SalesInvoiceDtBomType => {
      return {
        uid: randomId(),
  ...
  const data: SalesInvoiceDtType = {
    uid: randomId(),
    id: item.sales_invoice_id ?? null,
...
export function generateSalesInvoiceDt(
  checkSelected: FormSalesInvoiceDtRefType[],
  checkOpened: SalesInvoiceRefType,
  checkMain: SalesInvoiceDtType[],
): SalesInvoiceDtType[] {
  let newRefItems: SalesInvoiceDtType[] = []
  let updatedList: SalesInvoiceDtType[] = []

  checkSelected.forEach((dt: FormSalesInvoiceDtRefType) => {
...
export function updateSalesInvoiceRefsModalFromMain(
  checkMain: SalesInvoiceDtType[],
  checkOpened: SalesInvoiceRefType,
  checkProducts: FormSalesInvoiceDtProductListType[]
): any[] {
  let updatedList: any[] = [];

  let selectedRefList: SalesInvoiceDtType[] = checkMain.filter((itemMain: SalesInvoiceDtType) => {
...
export function initCheckedSalesInvoiceDt(
  checkMain: SalesInvoiceDtType[],
): SalesInvoiceDtType[] {
  let updatedList: SalesInvoiceDtType[] = []

  checkMain.forEach((mainItem: SalesInvoiceDtType, iMainItem: number) => {
    updatedList[iMainItem] = {
      ...
      sales_invoice_dt_boms: mainItem.sales_invoice_dt_boms || mainItem.so_dts_boms || []
    }
  })
